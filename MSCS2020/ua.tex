...
unique sort, so we use the unit type \codigo{⊤} with its sole constructor
\codigo{tt|. We define a family indexed on |List ⊤ x ⊤}, with two constructors,
corresponding with the operations: a 0-ary operation \codigo{e}, and a binary
operation \codigo{∙} (note that constructors can
