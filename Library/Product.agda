...
module IndexedProduct {ℓ₁ ℓ₂ ℓ₃}
  ...
  π : (i : I) → Homo Πalg (A i)
  π i = record
    { ′_′ = λ _ → record { _⟨$⟩_ = _$ i ; cong = _$ i } 
    ; cond = λ {_} {s} f as → Setoid.refl (A i ⟦ s ⟧ₛ)
    }

  module ProdMor {ℓ} {ℓ'} (C : Algebra {ℓ} {ℓ'} Σ)  where
    ...
    ⟨ f ⟩ = record
      ...
      hom-cond {s} ft cs i
        rewrite map-compose <f> (λ _ x → x i) cs = cond (f i) ft cs
...
module BinaryProduct {ℓ₁ ℓ₂}
       ...
       iso×-2→ = record
         ...
         where
         _$h_ : ∀ {s} → (x : Setoid.Carrier (A×B ⟦ s ⟧ₛ)) → (i : Bool) →
         ...
         condh : Hom.homCond A×B Πalg (λ s → record
           { _⟨$⟩_ = _$h_
           ; cong = λ {i} {j} → congh })
         condh {ar = ar} f as false
           rewrite map-compose (λ x → _$h_) (λ _ x → x false) as =
             Π.cong (B ⟦ f ⟧ₒ) (Setoid.refl (_⟦_⟧ₛ B ✳ ar))
         condh {ar = ar} f as true
           rewrite map-compose (λ x → _$h_)  (λ _ x → x true) as =
             Π.cong (A ⟦ f ⟧ₒ) (Setoid.refl (_⟦_⟧ₛ A ✳ ar))
