...
data op-lattice : List ⊤ × ⊤ → Set where
  sup   : op-lattice ((tt ∷ [ tt ]) ↦ tt)
  inf   : op-lattice ((tt ∷ [ tt ]) ↦ tt)

Σ-lattice : Signature
Σ-lattice = record { sorts = ⊤ ; ops = op-lattice }


{- Booleans with false and ∨ are a monoid. -}
module BoolLattice where
  ...
  ∨-Monₒ : ∀ {ar s } → ops Σ-lattice (ar , s) → (∨-Monₛ ✳ ar) ⟶ ∨-Monₛ s
  ∨-Monₒ sup = record { _⟨$⟩_ = ? ; cong = ? }
  ∨-Monₒ inf = {!!}
